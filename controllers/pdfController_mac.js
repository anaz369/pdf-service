...
class PdfController {
  ...
  async generatePdfWithRazorView(req, res) {
    ...
    try {
      ...
      console.log(`Template No: ${pdfRequest.template_no}`);
      ...
      // ── Task 1: QR Code ──────────────────────────────────
      // Skip QR for thermal (201) and pre-print (202) templates
      // pdfRequest.qrcode is a ZATCA TLV base64 string → converted to PNG QR image
      const needsQr = pdfRequest.template_no !== 201 && pdfRequest.template_no !== 202;
      ...
      switch (pdfRequest.template_no) {
        case 201:
          ...
          paperConfig = { useHeaderFooter: false };
          break;

        case 202:
          // Pre-printed form — 220×280mm custom paper, background image overlay
          paperConfig = { width: '220mm', height: '280mm', useHeaderFooter: false };
      ...
      // isThermal = no format, no width  → 80mm thermal roll
      // isCustom  = no format, has width → pre-printed custom size (220×280mm)
    ...
    <script>
      ...
      if (useHeaderFooter) {
        ...
        pdfOptions.margin = { top: '120px', bottom: '5px', left: '5px', right: '5px' };
